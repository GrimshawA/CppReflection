...
token get_tok(const std::string& program_source, int& i, int& line)
{
...
void lexer::analyze(const std::string& src)
{
    if (src.empty())
        return;
    ...
    token t = get_tok(src, i, line);
    while (t.type != RZTK_EOF) {
        tokens.push_back(t);
        t = get_tok(src, i, line);
    }
